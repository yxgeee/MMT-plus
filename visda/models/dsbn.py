...
class DSBN2d(nn.Module):
    def __init__(self, planes, eps=1e-5, momentum=0.1):
        super(DSBN2d, self).__init__()
        self.num_features = planes
        self.BN_S = nn.BatchNorm2d(planes, eps=eps, momentum=momentum)
        self.BN_T = nn.BatchNorm2d(planes, eps=eps, momentum=momentum)
...
class DSBN1d(nn.Module):
    def __init__(self, planes, eps=1e-5, momentum=0.1):
        ...
        self.num_features = planes
        self.BN_S = nn.BatchNorm1d(planes, eps=eps, momentum=momentum)
        self.BN_T = nn.BatchNorm1d(planes, eps=eps, momentum=momentum)
...
def convert_dsbn(model):
    for _, (child_name, child) in enumerate(model.named_children()):
        ...
        if isinstance(child, nn.BatchNorm2d):
            m = DSBN2d(child.num_features, child.eps, child.momentum)
            m.BN_S.load_state_dict(child.state_dict())
            m.BN_T.load_state_dict(child.state_dict())
            setattr(model, child_name, m)
        elif isinstance(child, nn.BatchNorm1d):
            m = DSBN1d(child.num_features, child.eps, child.momentum)
            m.BN_S.load_state_dict(child.state_dict())
            m.BN_T.load_state_dict(child.state_dict())
            setattr(model, child_name, m)
        else:
    ...
    for _, (child_name, child) in enumerate(model.named_children()):
        ...
        if isinstance(child, DSBN2d):
            m = nn.BatchNorm2d(child.num_features,
                    eps=child.BN_S.eps, momentum=child.BN_S.momentum)
            if use_target:
