...
class Target_Test(BaseImageDataset):
    ...
    def __init__(self, root, verbose=True):
        ...
        self.query = self._process_dir(self.query_dir,
                                        list_file=osp.join(root, 'index_test_query.txt'))
        self.gallery = self._process_dir(self.gallery_dir, 
                                        list_file=osp.join(root, 'index_test_gallery.txt'))
        if verbose:
            print("=> target_test loaded")
    ...
    def _process_dir(self, dir_path, list_file=None):
        if list_file:
            f = open(list_file, "r")
            img_paths = []
            for line in f:
